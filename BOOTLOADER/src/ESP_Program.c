...
static volatile u8  CheckData[200] ={0};
...
void ESP_vidGetMetaData(void)
{
	...
	while((CheckData[i+1] != '\r') && (CheckData[i+1] != '\n'))
	{
		Size *= 10;
		Size += (CheckData[i+1] - '0');
		i++;
	}
	while( CheckData[i] != 'V' )
	{
	...
	{
		Version *= 10;
		Version += (CheckData[i+1] - '0');
		i++;
	}

	//ESP_vidConnectServerTCP((u8 *)"http://bootloader2.freevar.com" , (u8 *)"80");
}
...
void ESP_vidGetPageFromServer( u8   Copy_u8PageNum  )
{
	...
	{
		MUSART1_vidTransmitString((u8 *)"AT+CIPSEND=47\r\n");
		MSTK_vidSetBusyWait(500000);
		ESP_vidClearBuffer();

		Program_Flag = 1;
		MUSART1_vidTransmitString((u8 *)"GET http://fota.freevar.com/script.php?page=");
	...
	{
		MUSART1_vidTransmitString((u8 *)"AT+CIPSEND=48\r\n");
...
{
	ESP_vidClearBuffer();
	MUSART1_vidTransmitString((u8 *)"AT+CIPSEND=51\r\n");
	MSTK_vidSetBusyWait(500000);
	ESP_vidClearBuffer();


	MUSART1_vidTransmitString((u8 *)"GET http://fota.freevar.com/script.php?download=");
	MUSART1_vidTransmitChar(Copy_u8DownloadFlag + '0');
	MUSART1_vidTransmitString((u8 *) "\r\n");
